    ...
    public partial class Editor : Form
    {
        ...
        private void caminhoDeCustoMínimoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (g.getN() != 0)
            {
                ...
                string txt = Interaction.InputBox("Digite o nome da vértice de origem", "Caminho de custo mínimo");
                Vertice v1 = g.getVertice(txt);
                if (v1 != null)
                {
                    txt = Interaction.InputBox("Digite o nome da vértice de destino", "Caminho de custo mínimo");
                    Vertice v2 = g.getVertice(txt);
                    if (v2 != null)
                        g.caminhoMinimo(v1.getNum(), v2.getNum());
                    else
                        MessageBox.Show("Erro: não há vértices no grafo com esse nome!", "Mensagem", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                    MessageBox.Show("Erro: não há vértices no grafo com esse nome!", "Mensagem", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void excluirVérticeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            { //Se um vértice está selecionado
                g.DeleteVertice(); //Deleta Vertice
                g.desmarcarGrafo(); // reseta as cores 
            }
            else
                MessageBox.Show("Selecione um vertice!", "Mensagem", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        ...
        private void BtSobre_Click(object sender, EventArgs e)
        {
            MessageBox.Show("Editor de Grafos - 2025/1\n\n" +
                ...
                "72300574 - VICTOR AUGUSTO DIAS MENDES DO VALLE (co-autor)\n\n" +
